...
class NTPtime:
    ...
    def time(self):  # Local time offset in hrs relative to UTC
        ...
        try:
            ...
            if poller.poll(1000):  # time in milliseconds
                ...
                self.last_timestamp = max(val - self.NTP_DELTA + self.hrs_offset * 3600, 0)
    ...
    def loop(self): # loop to sync time
        offset = time.time() - self.last_timestamp
        print("loop: time=" + str(time.time()))
        print("loop: last_timestamp=" + str(self.last_timestamp))
        print("loop: offset=" + str(offset))
        if offset > (self.sync_interval):
            print("NTPtime: update time *********************************************** ")
            self.time()
